...
public class TabCommand implements CommandExecutor, TabCompleter {
    ...
    private static final List<String> TOGGABLE_MODULES = List.of("tablist", "nametags", "scoreboard", "chat");
    ...
    private boolean handleReload(CommandSender sender) {
        ...
        // Мгновенное применение изменений ко всем игрокам
     ...
     * Теперь: при выключении сбрасывает эффекты, при включении применяет сразу.
     */
    private boolean handleToggle(CommandSender sender, String[] args) {
        ...
        if (newState) {
            // Включили: применяем сразу ко всем
            for (Player player : Bukkit.getOnlinePlayers()) {
                plugin.getAdapter().queueUpdate(player);
            }
        } else {
            // Выключили: сбрасываем то, что уже отправлено клиентам
            module.resetAll();
        }
