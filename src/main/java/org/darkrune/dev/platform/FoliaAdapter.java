...
public class FoliaAdapter implements PlatformAdapter {
    ...
    @Override
    public void runRepeatingGlobal(Runnable task, long delayTicks, long periodTicks) {
        // Folia: глобальный регион для задач, не привязанных к игроку
    ...
    private void flushUpdates() {
        ...
        while ((current = updateQueue.poll()) != null) {
            final Player player = current;  // effectively final для лямбды
            if (player.isOnline()) {
